...
- 영속성 컨텍스트 특징
    - 데이터 베이스의 기본키를 사용해 영속성 컨텍스트의 엔티티는 반드시 식별자를 가진다.
    - 조회시 식별자 값으로 구분한다.
...
2. 영속성 컨텍스트에 대해서 설명해주세요.
   - 영속성 컨텍스트란 객체를 영구적으로 저장하는 환경으로써, Database와 애플리케이션 사이에 위치하고 있습니다.
3. 영속성 컨텍스트의 장점에 대해서 설명해주세요.
   - 1차 캐시를 가지고 있어 DB에 접근하는 횟수를 줄여 조회 성능을 향상시킬 수 있습니다.
   - SQL문을 바로 DB로 전송하는 것이 아니라 쓰기 지연 저장소에 두었다가 commit이나 flush가 일어날 때 DB로 전송됩니다.
   - 객체의 동일성을 보장해줍니다.
   - 변경된 부분을 감지하여 줍니다.
4. 영속성 컨텍스트 환경에서의 flush()에 대해서 설명해주세요.
   - 1차 캐시에서 변경된 부분을 DB와 동기화하는 것입니다.
5. 즉시 로딩과 지연 로딩에 대해서 설명해주세요.
   - 즉시 로딩일 경우 엔티티와 연관관계를 맺고 있는 모든 데이터들을 한번에 조회해 옵니다.
   - 반면 지연 로딩의 경우 엔티티와 연관관계를 맺고 있는 모든 데이터들을 프록시 객체로 가져와있다가 해당하는 데이터를 사용할 경우 DB로 SQL을 날려 데이터를 가져옵니다.
6. 프록시에 대해서 설명해주세요.
   - 지연 로딩을 사용할 경우 데이터 베이스 조회시 비어있는 필드들을 채우기 위한 가짜 객체입니다.
   - 프록시 객체는 처음 한번만 초기화가 됩니다.
7. 영속성 전이가 무엇인가요?
8. 영속성 전이의 옵션에는 어떤 것이 있나요?
9. 고아 객체가 무엇인가요?
10. @Transactional이 어떻게 동작하나요?
11. 언제 @Transactional을 사용하나요?
12. 트랜잭션 전파에는 어떤 것이 있나요?
13. Spring에서의 트랜잭션 격리는 어떻게 적용하나요?
14. OSIV가 무엇인가요?
