...
## Collection
Java에서 컬렉션(Collection)이란 데이터의 집합, 그룹을 의미하며, JCF(Java Collections Framework)는 컬렉션 데이터를 구현하고 처리하는 여러 자료구조 클래스를 제공한다.

- Collection 인터페이스는 List, Set, Queue로 크게 3가지 상위 인터페이스로 분류할 수 있다. (stack은 List의 하위)
...
2. List : 순서가 있는 데이터의 집합으로 데이터의 중복을 허용한다.
- LinkedList
- Vector :  ArrayList와 동일한 내부구조를 가지고  동기화된 메소드로 구성되어 있다.
- ArrayList
- Stack

3. Queue : 순서가 있는 데이터의 집합으로 데이터의 중복을 허용한다.
- LinkedList
- PriorityQueue

4. Map : 키(Key), 값(Value)의 쌍으로 이루어진 데이터으 집합으로, 순서는 유지되지 않으며 키(Key)의 중복을 허용하지 않으나 값(Value)의 중복은 허용한다.
- Hashtable
- HashMap
- TreeMap
...
둘다 자바 List 인터페이스의 구현체로,

ArrayList는 배열 기반 리스트이다. 자료 구조의 특성상 배열은 인덱스로 데이터를 접근하여 조회의 시간 복잡도는 O(1)이지만, 데이터를 삭제하고 추가하는데 원소의 위치를 시프트해서 조절하거나, 배열의 크기 이상으로 원소가 추가되는경우 배열의 크기를 조정하고 원소를 복사한다. 그런 이유로 삭제와 추가에 대한 시간 복잡도는 O(N)이다.

+)  random access (임의 접근) : 메모리의 어드레스(번지)만 지시하면 어느 부분에서도 즉시 기억 데이 타를 읽어낼 수 있는 호출 방식
+) 배열의 경우는 연속적으로 메모리상에 존재하고, 참조변수 하나당 4byte이기 때문에 시작배열 주소 + n * 데이터 타입의 크기를 통해서 원하는 인덱스에 쉽게 접근할 수 있다.
+) 용량이 초과 되면 기존 크기의 50% 만큼 확장한다.

LinkedList는 연결리스트 기반의 리스트이다. 내부적으로 양방향의 연결 리스트로 구성되어 있어서 참조하려는 원소에 따라 처음부터 순방향으로 또는 역순으로 순회할 수 있다. 자료 구조의 특성상 연결되어있는 노드의 조회는 빠르지만 특정 순서의 노드를 조회하기위해서는 순차 탐색을 진행해야 한다. 따라서 조회의 시간 복잡도는 O(N)이다. 삽입과 삭제의 경우 해당 노드의 포인터 값만 변경해주면 되기 때문에 시간 복잡도는 O(1)이다.

+) LinkedList는 저장해야하는 데이터의 개수가 많아질수록 데이터를 읽어 오는 시간, 접근 시간(access time)이 길어진다.

조회가 빈번하다면 ArrayList가 효율적이고, 데이터의 삽입, 삭제 작업이 빈번하면 LinkedList가 성능상 좋다.

### HashSet, LinkedHashSet, TreeSet

#### HashSet
내부적으로 HashMap을 사용하는 HashSet은 객체를 저장하기 전에 hashcode()메서드를 호출하여 얻어낸 hash값으로 기존에 저장된 객체의 hashcode와 비교한다. 만약 동일한 hashcode를 가진 객체가 있다면 equals()로 비교한다. 두 객체가 다르다면 LinkedList형태로 데이터를 추가한다. HashSet은 삽입 순서를 유지하지 않고 최대 한 개의 Null을 허용한다. 삽입/삭제/contains의 복잡도는 O(1)이다.

#### LinkedHashSet
HashSet과 다르게 데이터의 저장 순서를 유지한다. 순서 유지를 위해 포인터 값을 저장하므로 HashSet에 비해 약간 느린 성능을 보인다. 삽입/삭제/contains의 복잡도는 O(1)이다.

#### TreeSet 
특정 조건에 맞춰 데이터를 정렬하여 저장한다. (기본적으로 오름차순으로 데이터를 정렬) 내부적으로 Red-Black Tree를 사용한다. 삽입과 삭제 과정에서 재정렬이 이뤄지기때문에 시간 복잡도는 O(logN)이며 조회시에도 트리를 탐색해야하므로 O(logN)의 시간이 걸린다. Null값은 허용되지 않는다. 내부적으로 TreeMap을 사용한다.


저장 순서가 유지되야하면 LinkedHashSet, 정렬되어야 한다면 TreeSet, 이외에는 HashSet을 사용하는 것이 성능상 좋다.

### HashTable, HashMap, LinkedHashMap, TreeMap

#### HashTable
Key, Value형태로 데이터를 저장할 수 있는 구조로서 Thread safe이 보장된다. 삽입 순서가 유지되지 않고 동기화로 인해 HashMap보다는 느리다. Key 값으로 Null을 허용하지 않습니다. 조회/삭제/삽입의 시간 복잡도는 O(1)이다.

##### HashMap
thread safe하지 않고 삽입 순서도 보장되지 않는다. Key값으로 최대 한 개의 Null값을 허용한니다. 조회/삭제/삽입의 시간 복잡도는 O(1)이다

#### LinkedHashMap
 삽입 순서를 유지한다. 순서 유지를 위해 포인터 값을 저장하므로 HashMap에 비해 약간 느린 성능을 보인다. 조회/삭제/삽입의 시간 복잡도는 O(1)이다.
 
#### TreeMap
특정 조건에 맞춰 데이터를 정렬하여 저장한다. 내부적으로는 이진 트리를 사용하며 삽입, 삭제시 트리를 정렬해야 하므로 시간 복잡도는 O(logN)이며 조회시에도 트리를 탐색해야하므로 O(logN)의 시간이 걸린다. Null값은 허용되지 않는다.
