...
(관점 지향 프로그래밍) AOP는 여러 오브젝트에 나타나는 공통적인 부가 기능을 모듈화하여 런타임시 로직을 재사용하는 기법

- 단일 책임 원칙(SRP)을 지킬 수 있다. (비즈니스 로직에서 횡단 관심사 분리 +  유지 보수 유리 + 중복코드 제거)
- OOP의 경우 비즈니스 로직의 모듈화가 핵심이고, Aop가 OOP답게 사용할 수 있도록 한다.
...
- Join point : 어드바이스가 적용 가능한 위치

- weaving : 지정된 객체에 aspect를 적용해서 새로운 프록시 객체를 생성하는 과정

- Point cut : 어드바이스를 적용할 타겟의 메서드를 선별하는 정규표현식, 어노테이션으로도 가능하다.
...
- 클래스 로드 시 : .class 파일이 클래스 로더에 의해 메모리에 올릴 때 적용

- 프록시 패턴 : 타겟 클래스를 런타임시 부가 기능을 제공하는 프록시로 감싸서 실행 
(Spring AOP 방식 : IoC, DI 기반이기 때문에 가능)



### Spring AOP vs AspectJ
- Join Point : AspectJ 에서는 메서드, 필드 접근, 객체 생성 등 다양하게 Join Point가 적용되지만 Spring AOP에서는 간단하게 메서드 레벨 만 제공된다.

- weaving 시점 : Spring AOP는 런타임 시에만 가능하고(스프링 컨테이너가 객체 생성을 관리해주기 때문에 가능/ Bean 객체만 적용 가능) AspectJ는 런타임에는 제공하지 않고 컴파일이나 로드 타임에만 허용 한다. (모든 Java 오브젝트에 적용 가능)

### Spring AOP 구현

- XML 설정 파일에 <aop:config>를 이용해서 어드바이스와 포인트컷을 설정
...
+) 대표적인 예시는 @Transactional, @Cacheable이 있다.


#### Service 계층에서 private 메소드에 @Transactional 을 붙였을 때 작동을 하지 않는 이유
...
Spring에서는 ProxyFactory 라는 기능을 사용하여 타겟 클래스의 인터페이스 구현 여부에 따라  JDK Dynamic Proxy 를 쓸지, CGLIB 을 쓸지 정한다. Spring boot는 이와 상관없이 CGLIB방식이 default이다.

#### Proxy 

- 기능: 타깃 과 같은 메서드를 구현하고 있다가 메서드가 호출되면 타깃 오브젝트로 위임해 지정한 요청에 대해 부가 기능을 수행

- 동작 과정 : 클라이언트 -> 프록시 -> 타깃

- 문제점
  1. 매번 새로운 클래스를 정의해야 한다.
  2. 타깃의 인터페이스를 구현핟고 위임하는 코드를 작성하기 번거롭다.
  3. 부가기능 코드의 중복이 생긴다.

- 

#### JDK Dynamic Proxy
프록시 팩토리에 의해 런타임 시 인터페이스 정보를 제공하면 해당 인터페이스를 구현한 클래스 오브젝트를 자동으로 생성 -> 인터페이스를 구현한 class에 대해서만 적용이 가능한 방법.

- Java reflection을 사용해 target class의 method를 invoke한다. 
- JDK 동적 프록시에 적용할 로직은 InvocationHandler 인터페이스를 구현하여 작성하면 된다.

- 장점 :
그 전에는 인터페이스 만큼 프록시 객체를 직접 생성해줘야 했지만, JDK Dynamic Proxy는 알아서 생성해 준다.

- 단점 :
- advise의 적용 대상인지와 상관 없이 모든 method call마다 invoke가 수행되므로 성능이 떨어진다.
- 인터페이스 없이 클래스만 존재하는 경우에는 적용할 수 없다.

+) 동적 프록시(Dynamic Proxy) : 개발자가 직접 프록시 클래스를 만들 필요 없이, 프록시 객체를 동적(Runtime)으로 생성해 주는 기술

#### CHLib
바이트코드를 조작해서 동적으로 프록시 객체를 생성하는 라이브러리이다.

- 클래스 상속을 이용하여 인터페이스가 존재하지 않아도 가능하다. 
  - final, private와 같이 overriding이 불가능한경우 사용할 수 없다.
