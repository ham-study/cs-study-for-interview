...
## 가상 메모리 개요
가상 메모리는 물리 메모리의 크기와 상관없이 프로세스에 커다란 메모리 공간을 제공하는 기술이다. 가상 메모리를 통해 프로세스는 운영체제가 어디에 있는지, 물리 메모리의 크기가 어느정도인지 신경쓰지 않고 메모리를 마음대로 사용할 수 있다.

### 가상 메모리의 크기와 주소
가상 메모리 시스템의 모든 프로세스는 자신이 어디에 위치했는지와 관련 없이 항상 0번지부터 주소가 시작한다. 이론상으로 가상 메모리 크기는 무한대이지만 실제로는 컴퓨터 시스템이 가진 물리 메모리의 최대 크기로 한정된다.그리고 물리 메모리의 부족한 크기를 하드 디스크의 일부 공간인 스왑영역을 사용하여 보완한다. 

가상 메모리 시스템에서 메모리 관리자는 물리 메모리와 스왑 영역을 합쳐서 프로세스가 사용하는 가상 주소를 실제 메모리의 물리 주소로 변환하는데 이를 동적 주소 변환이라 한다.

## 페이징 기법
페이징 기법은 고정 분할 방식을 이용한 가상 메모리 관리 기법으로, 물리 주소 공간을 같은 크기로 나누어 사용한다.

물리 메모리의 조각을 프레임이라 하고, 가상 메모리 조각을 페이지라고한다. 모든 페이지의 위치 정보는 페이지 매핑 테이블에 기록되어있다.

### 페이지 테이블 관리
- 페이지 테이블에는 페이지-프레임의 매핑 정보가 기록되어 있다.
- 모든 프로세스는 각 하나씩 페이지 테이블을 가지고 있다
- 메모리 관리자가 자주 사용하는 자료구조이므로 운영체제의 일부분에 모아둔다
- 페이지 테이블의 크기가 작지 않기 때문에 페이지 테이블의 크기를 적절히 유지하는 것이 핵심이다.
- 페이지 테이블에 빠르게 접근하기 위해 각 페이지 테이블의 시작 주소를 페이지 테이블 기준 레지스터에 보관한다

### 페이지 테이블 매핑 방식
- 직접 매핑
  - 모든 페이지 테이블을 물리 메모리에 가지고있는 가장 단순한 방식이다.
  - 모든 페이지를 물리 메모리에 가지고 있기 때문에 매우 빠르다
- 연관 매핑
  - 전체 페이지 테이블을 스왑 영역에 두고, 페이지 테이블의 일부를 물리 메모리에 가져오는 방식이다.
  - 페이지 테이블의 일부를 변환 색인 버퍼(Translation Look-aside Buffer)라고 부른다
  - 페이지 번호가 TLB에 있을 경우 TLB hit, 없을 경우 TLB miss라고 한다
  - 메모리를 절약할 수 있지만, TLB 미스가 빈번하게 발생하는 경우 성능이 떨어질 수 있다
  - TLB를 모두 검색 해야하기 때문에 TLB 미스가 발생하면 주소 변환이 느려진다.
- 집합-연관 매핑
  - TLB를 모두 검색 해야한다는 점을 개선한 방식이다
  - 페이지 테이블을 같은 크기의 여러 묶음으로 나누고, 각 묶음의 시작 주소를 가진 디렉터리 테이블을 새로 만들어 관리한다
  - 전체 테이블은 스왑 영역에 위치하고, 일부 테이블은 묶음 단위로 메모리에 로드한다.
  - 디렉터리 테이블을 먼저 살펴보면 원하는 페이지가 메모리에 있는지 확인할 수 있다
- 역매핑
  - 페이지 테이블이 메모리 공간을 많이 차지하는 단점을 개선한다. 
  - 물리 메모리의 프레임 번호를 기준으로 테이블을 작성한다
    - 물리 메모리가 어떤 프로세스의 어떤 페이지를 가지고 있는지를 테이블 형태로 구성하는 것이다
  - 프로세스의 수와 관련 없이 항상 일정한 테이블의 크기를 유지한다.
  - 페이지 테이블을 다 검사한 후에야 저장장치에 접근하기 때문에 검색 시간을 낭비하는 단점이있다.

## 세그먼테이션 기법
가변 분할 방식을 사용하는 가상 메모리 관리 기법으로 물리 메모리를 프로세스의 크기에 따라 가변적으로 나누어 사용한다. 세그먼테이션 기법도 세그먼테이션 테이블이라는 매핑 테이블을 사용한다.

세그먼테이션 테이블에는 세그먼트의 크기를 나타내는 limit과 물리 메모리상 시작 주소를 나타내는 address가 있다. 프로세스가 limit을 벗어난 주소에 접근하면 프로세스를 강제 종료하는데 이를 트랩이라고한다.

## 세그먼테이션-페이징 혼용 기법
페이징 기법은 고정 분할 방식으로 메모리 관리가 수월한 반면 페이징 테이블의 크기가 크다. 세그먼테이션 기법은 테이블의 크기는 작게 유지할 수 있지만 외부 단편화로 인해 추가적인 관리가 필요하다. 세그먼테이션-페이징 혼용 기법은 두 가지 방법의 장점만 취한 기법이다.

### 메모리 접근 권한
- 읽기, 쓰기, 실행, 추가 권한이있다.
- 프로세스의 코드 영역은 자기 자신을 수정하는 프로그램은 없기 때문에 읽기/실행 권한만 가진다
- 프로세스의 데이터영역에서 일반적인 변수는 읽기/쓰기 권한을 상수는 읽기 권한을 가진다.
- 메모리 접근 권한 검사는 가상 주소에서 물리 주소로 변환이 일어날 때마다 실행된다.

### 세그먼테이션-페이징 혼용 기법 도입
- 페이지로 분할된 가상 주소 공간에서 서로 관련있는 영역을 하나의 세그먼트 영역으로 묶어 세그먼테이션 테이블로 관리한다
- 세그먼트를 구성하는 페이지를 해당 페이지 테이블로 관리한다
- 각 세그먼테이션 테이블은 자신과 관련된 페이지 테이블의 시작주소를 나타낸다.
- 권한 비트와 같은 중복 데이터를 세그먼테이션 테이블에서 관리하여 테이블의 크기를 줄일 수 있다.

사용자가 특정 주소를 요청했을 때 세그먼테이션 테이블에서 오류가 없는지, 접근 권한은 가지고 있는지 확인하고 이를 통과하면 연결된 페이지 테이블로가서 원하는 프레임을 찾는다. 만약 페이지가 없으면 스왑 영역에서 해당 페이지를 가져온다.

## 캐시 매핑
캐시도 페이지 테이블 매핑 방식과 비슷하게 사용된다. 다만 페이지 테이블의 직접 매핑은 물리 메모리에 모든 페이지 테이블을 가지고 있고, 캐시의 직접 매핑은 캐시된 데이터가 항상 같은 자리에 올라오기 때문에 캐시 테이블 전체를 찾아보지 않아도 캐시 히트나 미스를 바로 알 수 있다.

### 캐시 직접 매핑
메모리는 N개의 페이지로, 캐시는 M개의 페이지로 구성된다고 가정한다. 메모리가 캐시보다 훨씬 크기 때문에 N이 M보다 훨씬 크다.  메모리의 N개 페이지를 캐시의 M개 페이지로 나누면 (N/M)개의 블록이 된다. 

그리고 캐시에서는 어떤 블록에서 올라온 페이지인지만 확인하면된다. 따라서 캐시에서는 블록 번호를 명시하는데 이를 태그라고한다. 태그에는 블록의 시작 위치가 저장되어있고, 페이지는 블록내의 위치와 동일하게 캐시에 올라온다. 따라서 캐시 히트인지 미스인지 바로 확인할 수 있는 것이다.

캐시 직접 매핑의 특징은 다음과 같다
- 메인 메모리의 페이지가 캐시의 정해진 위치에만 들어갈 수 있다
- 페이지가 같은 위치에만 올라오기 때문에 자리 다툼이 발생할 수 있다
  - 이를 해결하기 위해 연관 매핑을 사용한다

### 캐시 연관 매핑

캐시 연관 매핑에서는 메모리 워드가 캐시의 어느 위치에도 자유롭게 올라갈 수 있다. 자리 다툼은 해결할 수 있지만, 캐시를 모두 검색한 후에야 캐시 미스인지 확인할 수 있다.

### 캐시 집합-연관 매핑
직접 매핑하는 캐시 메모리를 K개로 나눈다. 같은 끝자리를 가진 캐시 메모리도 K개가 되기 떄문에 직접 매핑의 자리 다툼이 완화된다. 그리고 집합 내에서 직접 매핑을 사용하기 때문에 캐시 전체를 검색하지 않고 캐시 미스를 판단할 수 있다.

## 요구 페이징
사용자가 요구할 때 페이지를 메모리로 가져오는 것을 요구 페이징이라 한다. 사용자가 어떤 기능을 요구할 때 해당 프로세스의 부분을 메모리로 가져오면 메모리의 절약, 메모리의 효율적 관리, 프로세스의 응답 속도 향상 같은 효과를 볼 수 있다.

### 페이지 테이블 엔트리의 구조
페이지 테이블에 저장되어 있는 데이터들을 페이지 테이블 엔트리라고한다. 페이지 테이블 엔트리에는 페이지 번호와 프레임 번호뿐 아니라 플래그 비트가 포함되어있다. 플래그 비트는 다음과 같이 구성된다.
...
- 유효 비트(현재 비트)
  - 페이지가 실제 메모리에 있는지 알려주는 비트
- 읽기, 쓰기, 실행 비트

### 페이지 부재(Page fault)
프로세스가 페이지를 요청했을때 그 페이지가 메모리에 없는 상황을 페이지 부재라고한다. 페이지 부재는 페이지 테이블 엔트리의 플래그 중 유효 비트로 확인할 수 있다. 유효 비트가 0인경우 페이지가 메모리에 위치하고, 1인 경우 페이지가 스왑 영역에 위치하는 것이다.

페이지 부재가 발생한 경우 페이지를 물리 메모리의 비어있는 공간으로 가져오고 페이지 테이블에 해당 프레임의 번호가 저장되고 유효 비트가 0으로 변경된다. 물리 메모리로 페이지를 가져올 때 만일 프레임이 비어있으면 문제가 되지 않지만, 프레임의 빈 공간이 없을 경우에는 페이지를 선택해서 스왑 영역으로 내보내야 한다. 어떤 페이지를 스왑 영역으로 내보낼지 결정하는 것을 페이지 교체 알고리즘이라한다. 그리고 페이지 교체 알고리즘으로 교체될 페이지를 대상 페이지라한다.

### 지역성
페이지 교체 알고리즘을 통해 페이지를 스왑 영역으로 옮길 때 자주 사용할 페이지를 옮기게되면 시스템 성능에 저하를 가져온다. 따라서 페이지 교체 알고리즘은 지역성을 바탕으로 대상 페이지를 선정한다. 지역성은 기억장치에 접근하는 패턴이 특정 영역에 집중되는 성질을 말한다.
...
- 순차적 지역성
  - 여러 작업이 순차적으로 진행될 가능성이 높다

## 페이지 교체 알고리즘
스왑 영역으로 보낼 대상 페이지를 선정하는 알고리즘으로, 메모리에서 앞으로 사용할 가능성이 낮은 페이지를 대상 페이지로 선정해야 페이지 부재를 줄이고 시스템의 성능을 향상한다.

### 무작위 페이지 교체 알고리즘
대상 페이지를 특별한 기준 없이 무작위로 선정하여 교체하는 알고리즘이다. 지역성이 고려되지 않아 알고리즘의 성능이 좋지않다.

### FIFO 페이지 교체 알고리즘
가장 먼저 들어온 페이지를 대상 페이지로 선정한다. 페이지가 메모리에 올라온 시점만 고려하기 때문에 자주 사용하는 페이지가 교체될 수 있다.

### 최적 페이지 교체 알고리즘
메모리가 앞으로 사용할 페이지를 미리 살펴보고, 페이지 교체 선정 시점에서 가장 멀리 떨어진 페이지를 대상 페이지로 선정한다. 미래의 패턴을 미리 안다는 것은 불가능하여 실제로 구현할 수 없다.

### LRU 페이지 교체 알고리즘
가장 오랫동안 사용되지 않은 페이지를 대상 페이지로 선정하는 알고리즘이다. 최근 최소 사용 페이지 교체 알고리즘으로 불린다. LRU의 페이지 교체 알고리즘의 기준은 시간, 카운터, 참조 비트를 사용할 수 있다.

- 시간에 기반한 구현
  - 가장 오랫동안 사용하지 않은 페이지를 대상 페이지로 선정한다.
  - 페이지에 접근한 시간을 기록해야 한다
  - 메모리 사용 패턴이 달라지면 FIFO 알고리즘만큼 성능이 저하되기도 한다

- 참조 비트 시프트 구현
  - 각 페이지의 일정 크기의 참조 비트를 만들어 사용하는 방식이다.
  - 참조 비트의 초기값은 0이고 페이지에 접근할 때마다 1로 바뀐다
  - 또한 참조 비트는 주기적으로 한 칸씩 오른쪽으로 이동한다
  - 참조 비트 중 가장 작은 값을 대상 페이지로 선정한다

### LFU 페이지 교체 알고리즘
페이지마다 사용된 횟수를 세어 가장 적은 페이지 영역을 스왑 영역으로 옮긴다. 최소 빈도 사용 알고리즘으로 불린다. 처음 메모리에 올라온 순간 페이지의 사용 빈도가 1이고 사용될 때마다 빈도가 증가한다. 그리고 페이지 교체가 필요할 때 가장 작은 빈도를 가진 페이지를 대상으로한다.

LRU와 LFU는 최적 근사 알고리즘으로 FIFO보다 우수하다. 하지만 LRU는 시간이나 참조 비트를 기록하기 위한 추가 공간이 필요하고, LFU는 접근 횟수를 기록하기 위한 별도의 메모리 공간이 필요하다는 단점이 있다. 

### NUR 페이지 교체 알고리즘
NUR(Not Used Recently)은 LFU, LRU 알고리즘과 성능은 비슷하면서 불필요한 메모리 공간 낭비를 해결한 알고리즘이다. 알고리즘에서 접근 시간이나 접근 빈도의 정확한 값을 저장하는 것이 공간만 차지할 뿐 큰 의미가 없다는 경향을 반영한 알고리즘으로 참조 비트, 변경 비트 2개만으로 미래를 추정한다.

- 참조 비트: 페이지에 접근하면 1이 된다
- 변경 비트: 페이지가 변경되는 1이 된다

모든 페이지의 초기 상태는 (0, 0)이다. 페이지에 읽기나 실행이 발생하면 (1, 0)이 되고, 변경이 발생하면 (0, 1)이 된다. 대상 페이지를 선정할 때는 참조 비트를 우선하고 다음으로 변경 비트를 확인한다. 즉, (0, 0) (0, 1) (1, 0) (1, 1) 순으로 대상 페이지로 선정된다. 모든 페이지가 (1, 1)이 되면 다시 모든 페이지를 (0, 0)으로 변경한다.

NUR 페이지 교체 알고리즘은 단 2bit만으로 LRU, LFU와 비슷한 성능을 내고 구현이 간단해 가장 많이 사용되고 있는 교체 알고리즘이다.

### FIFO 변형 페이지 교체 알고리즘

- 2차 기회 페이지 교체 알고리즘
  - 특정 페이지에 접근하여 페이지 부재 없이 서공할 경우 해당 페이지를 큐의 맨 뒤로 이동하여 대상 페이지에서 제외하는 알고리즘이다
    - 다시말해 성공한 페이지를 다시 큐의 맨 뒤로 보내 기회를 한 번 더 주는 것이다.
  - 최적 근사 페이지 교체 알고리즘과 성능이 유사하다
  - 큐를 유지하는 비용이 높고, 큐의 중간에 있는 값을 뒤로 이동시켜야 한다는 단점이 있다.
- 시계 알고리즘
  - 스왑 영역으로 옮길 대상 페이지를 가리키는 포인터를 사용한다
  - 포인터는 큐의 맨 바닥으로 이동하면 다시 처음으로 돌아온다. 
  - 참조 비트를 사용한다
  - 페이지가 필요할 경우 포인터가 가르키는 페이지를 대상 페이지로 선정한다
  - 페이지가 참조된 경우 참조 비트를 1로한다
  - 참조 비트가 1인 페이지는 포인터가 건너뛴다.
  - 건너뛰고 난 후에는 참조 비트가 0이 된다.
  - 추가적인 공간을 적게들지만 구현이 복잡하고 계산량이 많다는 단점이있다.

## 스레싱과 프레임 할당
하드디스크 입출력이 너무 많아져서 잦은 페이지 부재로 작업이 멈춘것 같은 상태를 스레싱(Threshing)이라고 한다.

스레싱은 메모리의 크기가 일정한 경우 멀티 프로그램 수와 밀접한 관련이 있다. 멀티 프로그래밍 정도가 너무 높으면 스레싱이 발생한다.

프로그램의 수가 적을 떄는 CPU 사용률이 계속 증가하다가 메모리가 꽉차면 CPU가 작업하는 시간보다 스왑 영역으로 페이지를 내보내고 새로운 페이지를 메모리로 가져오는 작업이 빈번해져서 CPU가 작업할 수 없는 상태에 이르는데, 이를 스레싱 발생 지점이라한다. 프로세스가 필요로 하는 메모리보다 물리 메모리가 작다면 스레실 발생 지점에 빨리 도달하여 컴퓨터가 전체적으로 느려진다. 그리고 메모리가 충분히 큰 상태라면 이후에 크기를 늘려도 작업 속도에 큰 영향을 미치지 못한다.

### 프레임 할당
프로세스에 프레임을 할당하는 방식에는 정적 할당과 동적 할당이 있다.

- 정적 할당: 프로세스 실행 초기에 프레임을 나누어 준 후 그 크기를 고정하는 것
  - 균등 할당: 프로세스의 크기와 상관 없이 사용 가능한 프레임을 모든 프로세스에게 동일하게 할당한다
    - 크기가 큰 프로세스가 필요한만큼 프레임을 할당받지 못하기 때문에 페이지 부재가 빈번하다
    - 크기가 작은 프로세스는 메모리 낭비가 발생한다
  - 비례 할당: 프로세스 크기에 비례하여 프레임을 할당한다
    - 프로세스 크기에 비례하여 프레임을 할당하기 때문에 균등 할당에 비해 현실적이다
    - 프로세스가 실행 중에 필요로 하는 프레임을 유동적으로 반영하지 못한다. 작은 프로세스라도 많은 메모리를 요구하는 경우가있다. 프로그램 자체는 작지만 비디오 플레이어는 크기가 큰 동영상을 재생해야한다
    - 사용하지 않을 메모리를 처음 부터 미리 확보하여 공간을 낭비한다. 크기가 큰 프로세스가  실행 중이지 않더라도 당장 필요없는 프레임을 미리 할당한다

- 동적 할당: 시시각각 변하는 요청을 수용하는 방식이 동적할당이다.
  - 작업 집합 모델: 지역성 이론을 바탕으로 하며, 가장 최근에 접근한 프레임이 이후에도 또 참조될 가능성이 높다는 가정에서 출발한다.
    - 일정시간 동안 참조된 페이지들을 집합으로 만들고, 이 집합에 있는 페이지들을 물리 메모리에 유지하여 프로세스의 실행을 돕는다.
    - 작업 집합 윈도우 크기에 따라서 시스템 성능이 달라진다. 
    - 작업 집합 모델이 어떤 프레임을 메모리에 유지할지는 알 수 있지만, 프로세스에 프레임을 얼마나 할당 할 지는 알 수 없어 스레싱 문제를 해결하지 못한다.

  - 페이지 부재 빈도: 페이지 부재 횟수를 기록하여 페이지 부재 비율을 계산하는 방식이다
    - 페이지 부재 비율이 상한선을 초과하면 할당한 프레임이 적다는 것을 의미하므로 프레임을 늘리고, 하한선 아래로 내려가면 메모리가 낭비되고 있다는 것이므로 프레임을 줄인다.


## 프레임 관련 이슈
### 전역 교체와 지역 교체

- 전역 교체: 물리 메모리의 모든 프레임을 대상으로 스왑 영역에 보낼 페이지를 찾는다
  - 페이지 교체가 다른 프로세스에 영향을 미친다.
  - 전체 시스템 입장에서는 전역 교체 방식이 지역 교체 방식보다 효율적이다
- 지역 교체: 프로세스와 관련된 프레임만 대상으로 스왑 영역에 보낼 페이지를 찾는다
  - 페이지 교체가 다른 프로세스에 영향을 미치지 않는다.
  - 전체 프로세스에 할당된 프레임 수가 변하지 않기 때문에 스레싱을 줄일 수 있다
  - 자주 사용하는 페이지가 스왑 영역으로 옮겨져 시스템의 효율이 떨어질 수 있다
    
### 페이지 테이블의 크기
- 페이지 테이블의 크기를 줄이기 위해서 한 페이지의 크기를 늘릴 수 있다
- 하지만 페이지의 크기가 늘어나면 내부 단편화로 인한 메모리 낭비가 발생할 수 있다

### 쓰기 시점 복사
- 프로세스의 생성을 위해 매번 새로운 데이터 공간을 만드는 것은 낭비다
- 예를 들어 크롬 브라우저를 세 개 실행하면 일단 메모리 공간을 공유한다
- 그리고 데이터의 변화가 있을 때 복사를 진행한다
- 쓰기 시점까지 복사를 지연하여 쓸데 없는 메모리 공간 낭비를 줄이는 것이다
